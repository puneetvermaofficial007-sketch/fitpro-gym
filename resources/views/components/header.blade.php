        ...
        <div x-data="{ open: false }" class="relative">
            ...
            <div x-show="open" @click.outside="open = false" x-transition class="absolute right-0 mt-2 w-80 rounded-xl border border-slate-200 bg-white shadow-lg">
                ...
                <div class="max-h-64 overflow-y-auto p-2">
                    @foreach([
                        ['warning', 'Membership expiring soon', '3 memberships expire within 7 days'],
                        ['danger', 'Payment pending', '2 invoices are overdue'],
                        ['info', 'New enquiry', 'Deepak Verma submitted enquiry'],
                    ] as [$type, $title, $msg])
                        <div class="flex gap-3 rounded-lg p-3 hover:bg-slate-50">
                            <div class="mt-0.5 h-2 w-2 shrink-0 rounded-full {{ $type === 'warning' ? 'bg-amber-500' : ($type === 'danger' ? 'bg-red-500' : 'bg-blue-500') }}"></div>
                            <div>
